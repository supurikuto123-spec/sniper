...
const pumpSimulator_1 = require("./pumpSimulator");
...
// インスタンス作成
const tradeManager = new tradeManager_1.TradeManager();
const simulator = new pumpSimulator_1.PumpSimulator();
// ===== HTTP APIエンドポイント =====
// 現在のポジション一覧
...
// ===== Pump.funシミュレーター連携 =====
// 新規トークン検出時（非同期対応）
simulator.on('token', async (event) => {
...
// 取引発生時（価格更新・非同期対応）
simulator.on('trade', async (event) => {
